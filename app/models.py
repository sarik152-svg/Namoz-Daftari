...
# ---------------------------------------------------------------- qur'on
# How many ayahs each sura has, in order, by the Kufan count the Uzbek mus'haf
# uses. Here so the server can refuse an ayah that does not exist — a 300 typed
# into Fotiha would otherwise sit in the log and read as a finished sura.
SURA_AYAHS = (
...
class QuranRead(BaseModel):
    """One entry: on this day, this member reached this ayah of this sura."""
...
class QuranReadCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class QuranDone(BaseModel):
    """A sura finished, on the day it was finished — so the points it is worth land
    in the period that contains it."""
